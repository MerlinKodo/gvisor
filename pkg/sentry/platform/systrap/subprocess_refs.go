...
const subprocessenableLogging = false

// obj is used to customize logging. Note that we use a pointer to T so that
// we do not copy the entire object when passed as a format parameter.
var subprocessobj *subprocess
...
// checking.
func (r *subprocessRefs) InitRefs() {
...
func (r *subprocessRefs) RefType() string {
	return fmt.Sprintf("%T", subprocessobj)[1:]
}

// LeakMessage implements refs.CheckedObject.LeakMessage.
func (r *subprocessRefs) LeakMessage() string {
	return fmt.Sprintf("[%s %p] reference count of %d instead of 0", r.RefType(), r, r.ReadRefs())
}

// LogRefs implements refs.CheckedObject.LogRefs.
func (r *subprocessRefs) LogRefs() bool {
	return subprocessenableLogging
}

// ReadRefs returns the current number of references. The returned count is
// inherently racy and is unsafe to use without external synchronization.
func (r *subprocessRefs) ReadRefs() int64 {
...
//
//go:nosplit
func (r *subprocessRefs) TryIncRef() bool {
...
//
//go:nosplit
func (r *subprocessRefs) DecRef(destroy func()) {
	v := r.refCount.Add(-1)
	if subprocessenableLogging {
		refs.LogDecRef(r, v)
	}
...
func (r *subprocessRefs) afterLoad() {
	if r.ReadRefs() > 0 {
		refs.Register(r)
	}
}
